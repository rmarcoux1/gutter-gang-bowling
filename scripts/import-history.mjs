...
//   season, week, date, opponent, player, string, score, strikes, spares, tens, orangePinsLeft
//
...
async function main() {
  ...
  // Create matches, then submit each row as a result.
  let matchCount = 0;
  let resultCount = 0;
  for (const group of matchGroups.values()) {
    ...
    console.log(`  Season ${group.season}, week ${group.week} vs ${group.opponent}: ${group.rows.length} results`);
  }

  console.log(`\nDone. Created ${matchCount} matches and ${resultCount} results.`);
}
