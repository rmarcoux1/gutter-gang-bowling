...
// - `money_owed` (a side-bet/dues field) is carried through as a `moneyOwed` column on each
//   row. It maps to the app's per-match Payment feature (one amountPaid per player per match,
//   not per-string), so `import-history.mjs` sums a player's `moneyOwed` across their strings
//   within a match and submits that as one payment, if the total is greater than 0.
...
async function main() {
  ...
  const header = "season,week,date,opponent,player,string,score,strikes,spares,tens,orangePinsLeft,moneyOwed";
  console.log(header);
